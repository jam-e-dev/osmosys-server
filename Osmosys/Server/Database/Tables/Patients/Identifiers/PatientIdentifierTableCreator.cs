...
namespace Server.Database.Tables.Patients.Identifiers
{
    public class PatientIdentifierTableCreator : IPatientIdentifierTableCreator
    {
        ...
        {
            const string sql = "create table if not exists patient_identifiers (pk bigserial primary key, patient_fk bigint, foreign key (patient_fk) references patients(pk), identifier_type_fk bigint, foreign key (identifier_type_fk) references identifier_types(pk), use text, system text, value text, period_start timestamp, period_end timestamp)";
